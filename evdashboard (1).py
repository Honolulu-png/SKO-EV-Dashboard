...
def load_default_data():
    ...
    if os.path.exists("attached_assets"):
        for file in os.listdir("attached_assets"):
            if file.endswith('.csv') and 'AI Camp' in file:
                csv_files.append(os.path.join("attached_assets", file))
    if csv_files:
        latest_csv = max(csv_files, key=os.path.getmtime)
        for enc in ["utf-8", "cp949", "latin1", "utf-8-sig"]:
            try:
                df = pd.read_csv(latest_csv, encoding=enc)
                st.sidebar.success(f"✅ 기본 CSV 로드 성공: {os.path.basename(latest_csv)} ({enc})")
                break
            except:
                continue
...
# ========================
# Monthly Columns + Data Cleaning (EVdashboard 방식)
# ========================
...
def get_m13_data(monthly_cols, base_month_str="Jun-25"):
    df_months = [pd.to_datetime(c.replace("(EV)", "").replace("\n", "").strip(), format="%b-%y") for c in monthly_cols]
    month_map = dict(zip(df_months, monthly_cols))
    ...
    return recent_13_months, latest_month, latest_month_dt

# 선택된 기준월로 M-13 데이터 재계산
recent_13_months, latest_month, latest_month_dt = get_m13_data(monthly_cols, selected_base_month)
...
if not st.session_state.search_performed:
    ...
    # ========================
    # Tab 2: M-13 History (Top 10+Others 정합 + 그래프 형태 수정)
    # ========================
    with tab2:
        ...
        st.subheader("🔋 Battery Supplier 판매량 추이 (Top 10 + Others)")
        ...
        others_sum = supplier_hist_full.loc[~supplier_hist_full.index.isin(top10_suppliers)].sum()
        if others_sum.sum() > 0:
            supplier_hist.loc["Others"] = others_sum
        ...
        st.subheader("🚘 Top Selling Model 판매량 추이 (Top 10 + Others)")
        ...
        others_sum = model_hist_full.loc[~model_hist_full.index.isin(top10_models)].sum()
        if others_sum.sum() > 0:
            model_hist.loc["Others"] = others_sum
    ...
    # ========================
    # Tab 3: Gemini 분석 (EVdashboard 방식으로 데이터 추출 정합화)
    # ========================
    with tab3:
        st.header("🤖 AI Assistant (Gemini)")

        if gemini_client and recent_13_months:
            st.info(f"기준월 **{selected_base_month}** 기준 최근 13개월 데이터를 Gemini AI가 분석합니다.")

            # EVdashboard 방식: M-13 데이터 추출
            analysis_df = filtered_df[['AutoGroup', 'Model', 'Battery Supplier', 'Type_2'] + recent_13_months]
            analysis_df = analysis_df.copy()

            # 수치 데이터 정합화 (NaN → 0, 음수 제거)
            for col in recent_13_months:
    ...
    다음 CSV 데이터는 기준월 {selected_base_month} 포함 최근 {len(recent_13_months)}개월 판매 데이터입니다.
